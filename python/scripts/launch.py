...
MAP_NAME_COMMAND[RESEARCH_AGENT_NAME] = (
    f"uv run --env-file {ENV_PATH_STR} -m stockbuddy.agents.research_agent"
)
MAP_NAME_COMMAND[AUTO_TRADING_AGENT_NAME] = (
    f"{AUTO_TRADING_ENV_PREFIX}uv run --env-file {ENV_PATH_STR} -m stockbuddy.agents.auto_trading_agent"
)
MAP_NAME_COMMAND[NEWS_AGENT_NAME] = (
    f"uv run --env-file {ENV_PATH_STR} -m stockbuddy.agents.news_agent"
)
MAP_NAME_COMMAND[STRATEGY_AGENT_NAME] = (
    f"uv run --env-file {ENV_PATH_STR} -m stockbuddy.agents.strategy_agent"
)
BACKEND_COMMAND = (
    f"cd {PYTHON_DIR_STR} && uv run --env-file {ENV_PATH_STR} -m stockbuddy.server.main"
)
