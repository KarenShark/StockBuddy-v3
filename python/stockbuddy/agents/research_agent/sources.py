...
async def web_search(query: str) -> str:
    ...
    """
    from stockbuddy.utils.model import create_model_with_provider
...
# ============================================================================
# 🆕 Hong Kong Market Data Functions
# These functions integrate HK-specific data adapters for HKEX stocks
# ============================================================================


async def get_hk_capital_flow(days: int = 30) -> Optional[dict]:
    """
    获取港股通资金流向数据 (Southbound/Northbound Capital Flow)

    此函数调用 HKConnectAdapter 获取南下资金（A股投资者买入港股）和
    北上资金（港股投资者买入A股）的流向数据。

    仅在配置启用时使用 (ENABLE_HKEX_INTEGRATION=true)。

    Args:
        days: 查询天数，默认30天

    Returns:
        Optional[dict]: 资金流向数据，包含:
            - southbound: 南下资金数据 {total_inflow, daily_data, trend}
            - northbound: 北上资金数据
            - net_balance: 净额
            - analysis: 分析文本
        如果功能未启用或获取失败，返回 None

    Example:
        >>> flow = await get_hk_capital_flow(days=30)
        >>> if flow:
        >>>     print(f"30天南下资金净流入: {flow['southbound']['total_inflow']}亿元")
    """
    try:
        from stockbuddy.adapters.assets.hkconnect_adapter import HKConnectAdapter

        adapter = HKConnectAdapter()
        summary = adapter.get_flow_summary(days=days)

        return summary

    except ImportError as e:
        # HKConnectAdapter 不存在（不应该发生，因为我们已经创建了）
        import logging

        logging.warning(f"HKConnectAdapter not available: {e}")
        return None
        ...
        logging.error(f"Failed to fetch HK capital flow: {e}")
        return None


async def get_ah_premium(ticker: str) -> Optional[dict]:
    """
    计算指定港股的AH股溢价率 (AH Share Premium)

    此函数调用 AHPremiumAdapter 计算在A股和H股同时上市的股票的价格溢价。
    ...
    Returns:
        ...
        如果不是AH股、功能未启用或获取失败，返回 None

    Example:
        >>> premium = await get_ah_premium("00939")
        >>> if premium:
        >>>     print(f"{premium['h_stock_name']} AH溢价率: {premium['premium_rate']:.2f}%")
        >>>     print(f"套利机会: {premium['arbitrage_opportunity']}")
    """
    try:
        from stockbuddy.adapters.assets.ah_premium_adapter import AHPremiumAdapter

        # 清理 ticker 格式
        clean_ticker = ticker.replace("HKEX:", "").strip()

        adapter = AHPremiumAdapter()

        # 首先检查是否是AH股
        if not adapter.is_ah_stock(clean_ticker):
            import logging

            logging.info(
                f"{clean_ticker} is not an AH stock, skipping premium calculation"
            )
            return None

        # 计算溢价
        premium_data = adapter.calculate_premium(clean_ticker)
        ...
        logging.warning(f"AHPremiumAdapter not available: {e}")
        return None

    except Exception as e:
        import logging

        logging.error(f"Failed to calculate AH premium for {ticker}: {e}")
        return None


async def get_hk_southbound_holdings(limit: int = 10) -> Optional[List[dict]]:
    """
    获取南下资金重仓股列表 (Top Southbound Holdings)

    此函数获取被南下资金买入最多的港股列表，用于识别北向资金偏好。
    ...
    Returns:
        Optional[List[dict]]: 重仓股列表，每项包含:
            - stock_code: 股票代码
            - stock_name: 股票名称
            - holding_amount: 持股金额（亿元）
            - holding_percent: 持股占比（%）
            - change_amount: 持股变化（亿元）

        如果功能未启用或获取失败，返回 None

    Example:
        >>> holdings = await get_hk_southbound_holdings(limit=5)
        >>> if holdings:
        >>>     for stock in holdings:
        >>>         print(f"{stock['stock_name']}: {stock.get('holding_amount', 0)}亿元")
    """
    try:
        from stockbuddy.adapters.assets.hkconnect_adapter import HKConnectAdapter

        adapter = HKConnectAdapter()
        holdings = adapter.get_top_southbound_holdings(limit=limit)
        ...
        logging.error(f"Failed to fetch southbound holdings: {e}")
        return None
