...
class ConfigManager:
    ...
    def primary_provider(self) -> str:
        ...
        # 2. Auto-detect based on available API keys and network reachability
        ...
        if auto_detect:
            # 🆕 智能自动选择（检测网络可达性）
            config_primary = self._config.get("models", {}).get(
                "primary_provider", "openrouter"
            )
            auto_config = self._config.get("models", {}).get("auto_selection", {})

            if config_primary == "auto" and auto_config.get("enabled", True):
                try:
                    from stockbuddy.adapters.models.auto_selector import (
                        auto_select_provider,
                    )

                    check_health = auto_config.get("check_health", True)
                    selected_provider, _ = auto_select_provider(
                        check_health=check_health
                    )

                    logger.info(f"🤖 Smart auto-selected provider: {selected_provider}")
                    return selected_provider

                except Exception as e:
                    logger.warning(
                        f"Smart auto-selection failed: {e}, falling back to simple selection"
                    )

            # 简单自动选择（基于API Key可用性）
            enabled_providers = self.get_enabled_providers()
            if enabled_providers:
                # Priority order for auto-selection (优化为国内友好）
                preferred_order = [
                    "openai",  # 优先 OpenAI（国内可用）
                    "moonshot",  # 月之暗面（国内服务）
                    "google",  # Google（亚太可用）
                    "siliconflow",  # Silicon Flow（国内服务）
                    "openrouter",  # OpenRouter（需要VPN）
                    "openai-compatible",
                    "azure",
                ]
