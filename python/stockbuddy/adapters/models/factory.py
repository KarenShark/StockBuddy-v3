...
from stockbuddy.config.manager import ConfigManager, ProviderConfig, get_config_manager

logger = logging.getLogger(__name__)

# Models that don't support custom temperature parameter
# These models only support the default temperature value (usually 1)
# Note: Model IDs can be in different formats:
# - Direct OpenAI: "gpt-5-mini-2025-08-07"
...
def model_supports_temperature(model_id: str) -> bool:
    ...
    This function handles different model ID formats:
    - Direct provider format: "gpt-5-mini-2025-08-07"
    - OpenRouter format: "openai/gpt-5-mini-2025-08-07"
    
    Args:
        model_id: Model identifier (can be with or without provider prefix)
        
    Returns:
        True if model supports temperature, False otherwise
    """
    if not model_id:
        return True  # Default to supporting if model_id is None
    
    model_id_lower = model_id.lower()
    
    # Extract the actual model name (remove provider prefix if present)
    # e.g., "openai/gpt-5-mini-2025-08-07" -> "gpt-5-mini-2025-08-07"
    if "/" in model_id_lower:
        # OpenRouter format: "provider/model-name"
        actual_model = model_id_lower.split("/", 1)[1]
    else:
        # Direct format: "model-name"
        actual_model = model_id_lower
    
    # Check if the actual model name matches any model without temperature support
    supports = not any(
        no_temp_model in actual_model 
        for no_temp_model in MODELS_WITHOUT_TEMPERATURE
    )
    
    # Log for debugging
    logger.info(
        f"[Temperature Check] model_id='{model_id}' -> "
        f"actual_model='{actual_model}' -> supports_temperature={supports}"
    )
...
class OpenRouterProvider(ModelProvider):
    ...
    def create_model(self, model_id: Optional[str] = None, **kwargs):
        ...
        logger.info(f"[OpenRouter] Params received: {params}")
        ...
        if model_supports_temperature(model_id) and temperature_value is not None:
            model_args["temperature"] = temperature_value
            logger.info(f"[OpenRouter] Adding temperature={temperature_value} to model args")
        else:
            logger.info(
                f"[OpenRouter] Skipping temperature parameter for model {model_id} "
                f"(supports={model_supports_temperature(model_id)}, value={temperature_value})"
            )
        
        # Add other optional parameters
        if params.get("max_tokens") is not None:
            model_args["max_tokens"] = params.get("max_tokens")
        if params.get("top_p") is not None:
            model_args["top_p"] = params.get("top_p")
        if params.get("frequency_penalty") is not None:
            model_args["frequency_penalty"] = params.get("frequency_penalty")
        if params.get("presence_penalty") is not None:
            model_args["presence_penalty"] = params.get("presence_penalty")
        
        return OpenRouter(**model_args)
...
class GoogleProvider(ModelProvider):
    ...
    def create_model(self, model_id: Optional[str] = None, **kwargs):
        ...
        # Only add temperature if model supports it
        if model_supports_temperature(model_id) and params.get("temperature") is not None:
            model_args["temperature"] = params.get("temperature")
...
class AzureProvider(ModelProvider):
    ...
    def create_model(self, model_id: Optional[str] = None, **kwargs):
        ...
        # Only add temperature if model supports it
        if model_supports_temperature(deployment_name) and params.get("temperature") is not None:
            model_args["temperature"] = params.get("temperature")
...
class OpenAIProvider(ModelProvider):
    ...
    def create_model(self, model_id: Optional[str] = None, **kwargs):
        ...
        # For compatibility, try max_completion_tokens first, then fall back to max_tokens
        max_tokens_param = params.get("max_completion_tokens") or params.get("max_tokens")
        
        # Some models (e.g., gpt-5-mini) don't support custom temperature, only default (1)
        # Check if model supports temperature parameter
        model_args = {
            "id": model_id,
            "api_key": self.config.api_key,
            "base_url": self.config.base_url,
            "max_completion_tokens": max_tokens_param,  # Use max_completion_tokens for GPT-5
        }
        ...
        # Only add temperature if model supports it
        model_supports_temperature = not any(
            no_temp_model in model_id.lower() for no_temp_model in models_without_temperature
        )
        
        if model_supports_temperature and params.get("temperature") is not None:
            model_args["temperature"] = params.get("temperature")
