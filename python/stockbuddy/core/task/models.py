...
class Task(BaseModel):
    ...
    # Note: status and pattern must be Optional to avoid JSON Schema strict mode conflict with $ref + default
    ...
    super_agent_conversation_id: Optional[str] = Field(
        None,
        description="Main conversation ID from Super Agent (for event routing). If set, events will be sent to both this and the task's own conversation_id.",
    )
    depends_on: List[str] = Field(
        default_factory=list,
        description="List of task IDs that must complete before this task can start. Empty list means no dependencies.",
    )
