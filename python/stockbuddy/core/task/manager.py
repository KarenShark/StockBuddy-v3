...
class TaskManager:
    ...
    async def cancel_task(self, task_id: str) -> bool:
        ...
        async with self._lock:
            ...
            if not task:
                logger.warning(
                    f"Cannot cancel task {task_id}: Task not found in memory. "
                    f"This may indicate the backend was restarted and in-memory tasks were lost. "
                    f"Currently {len(self._tasks)} tasks in memory."
                )
                return False

            if task.is_finished():
                logger.warning(
                    f"Cannot cancel task {task_id}: Task already finished with status {task.status}"
                )
                return False

            logger.info(f"Cancelling task {task_id} (title: {task.title})")
            task.cancel()
            self._update_task_no_lock(task)
            return True
