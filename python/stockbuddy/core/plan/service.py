...
from stockbuddy.core.plan.planner import ExecutionPlanner, UserInputRequest
...
class PlanService:
    ...
    def start_planning_task(
        ...
        Args:
            user_input: User input to plan for
            thread_id: Thread ID for this planning session
            callback: Callback for user input requests
        ...
        return asyncio.create_task(
            self._planner.create_plan(
                user_input, callback, thread_id, recommended_agents=recommended_agents
            )
        )
