...
from stockbuddy.core.task.models import ScheduleConfig, Task, TaskPattern
...
class _TaskBrief(BaseModel):
    ...
    task_id: str = Field(
        ..., 
        description="Task identifier for referencing in dependencies (e.g., 'task_1', 'task_2')"
    )
    title: str = Field(
    ...
    # Note: pattern must be Optional to avoid JSON Schema strict mode conflict with $ref + default
