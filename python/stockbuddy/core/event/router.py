...
from stockbuddy.core.types import BaseResponse, CommonResponseEvent
...
async def handle_status_update(
    ...
    if not event.metadata:
        logger.debug(f"⚠️  No metadata in event for task {task.task_id[:16]}")
        return RouteResult(responses)

    response_event = event.metadata.get("response_event")
    logger.debug(f"📊 Event metadata - response_event: {response_event}, super_agent_conv: {task.super_agent_conversation_id[:16] if task.super_agent_conversation_id else 'None'}")

    # Tool call events
    if state == TaskState.working and EventPredicates.is_tool_call(response_event):
        logger.info(f"🔧 Tool call detected for task {task.task_id[:16]}, super_agent_conv: {task.super_agent_conversation_id[:16] if task.super_agent_conversation_id else 'None'}")
        ...
        # Create response for sub-agent's own conversation
        responses.append(
        ...
        # If this is a sub-agent handoff, ALSO send to main conversation for real-time monitoring
        if task.super_agent_conversation_id:
            responses.append(
                response_factory.tool_call(
                    conversation_id=task.super_agent_conversation_id,
                    thread_id=thread_id,
                    task_id=task.task_id,
                    event=response_event,
                    tool_call_id=tool_call_id,
    ...
    if state == TaskState.working and EventPredicates.is_reasoning(response_event):
        logger.info(f"🧠 Reasoning detected for task {task.task_id[:16]}, super_agent_conv: {task.super_agent_conversation_id[:16] if task.super_agent_conversation_id else 'None'}")
        # Create response for sub-agent's own conversation
        responses.append(
        ...
        # If this is a sub-agent handoff, ALSO send to main conversation
        if task.super_agent_conversation_id:
            responses.append(
                response_factory.reasoning(
                    conversation_id=task.super_agent_conversation_id,
