"""FastAPI application factory for StockBuddy Server."""
...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        # Startup
        print(
            f"StockBuddy Server starting up on {settings.API_HOST}:{settings.API_PORT}..."
        )
        ...
        yield
        # Shutdown
        print("StockBuddy Server shutting down...")

    app = FastAPI(
        title="StockBuddy Server API",
...
def _add_routes(app: FastAPI, settings) -> None:
    ...
    async def home_page():
        return SuccessResponse.create(
            ...
            msg="Welcome to StockBuddy Server API",
        )
    ...
    # Include HK stock trading router
    try:
        from .routers.hk_stock import create_hk_stock_router

        app.include_router(create_hk_stock_router(), prefix=API_PREFIX)
    except Exception as e:
        print(f"Skip HK stock router because of import error: {e}")

    # Include HK stock strategy router
    try:
        from .routers.hk_stock_strategy import create_hk_strategy_router

        app.include_router(create_hk_strategy_router(), prefix=API_PREFIX)
    except Exception as e:
        print(f"Skip HK stock strategy router because of import error: {e}")
