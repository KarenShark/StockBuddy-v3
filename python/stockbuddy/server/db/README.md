# StockBuddy Database Initialization

This directory contains database-related code for StockBuddy Server, including model definitions, connection management, and initialization scripts.
...
Database configuration is defined in `stockbuddy/server/config/settings.py`:

- **DATABASE_URL**: Database connection URL, defaults to `sqlite:///./stockbuddy.db`
- **DB_ECHO**: Whether to output SQL logs, defaults to `false`

## Database Models

### Agent Model (Agent)
The Agent model stores information about all available AI agents in the StockBuddy system:
...
### Asset Model (Asset)
The Asset model represents financial assets in the StockBuddy system, including stocks, bonds, cryptocurrencies, and other investment instruments.
...
1. **Basic initialization**:
   ```bash
   cd /path/to/stockbuddy/python
   python3 -m stockbuddy.server.db.init_db
   ```

2. **Force re-initialization**:
   ```bash
   python3 -m stockbuddy.server.db.init_db --force
   ```

3. **Verbose logging**:
   ```bash
   python3 -m stockbuddy.server.db.init_db --verbose
   ```
...
```python
from stockbuddy.server.db import get_db, Agent, Asset
...
```python
from stockbuddy.server.db import get_database_manager, Agent
...
```python
from stockbuddy.server.db import init_database
