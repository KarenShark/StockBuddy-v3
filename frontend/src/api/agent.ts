...
import { STOCKBUDDY_AGENT } from "@/constants/agent";
...
export const useGetAgentInfo = (params: { agentName: string }) => {
  return useQuery({
    ...
    queryFn: async () => {
      // Return hardcoded data for SuperAgent
      if (params.agentName === "SuperAgent") {
        return Promise.resolve({ data: STOCKBUDDY_AGENT });
      }
