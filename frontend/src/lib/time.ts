...
export class TimeUtils {
  ...
  /**
   * Alias for fromUTCRelative for convenience
   * @param time - UTC time input
   * @returns Relative time string
   */
  static formatRelative(time: TimeInput): string {
    return TimeUtils.fromUTCRelative(time);
  }

  /**
   * Format time as datetime string
   * @param time - Time input
   * @returns Formatted datetime string
   */
  static formatDateTime(time: TimeInput): string {
    return TimeUtils.format(time, TIME_FORMATS.DATETIME);
  }
}
